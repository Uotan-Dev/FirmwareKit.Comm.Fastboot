...
public partial class FastbootDriver
{
    ...
    /// 下载签名数据并发送签名命令。</para>
    /// </summary>
    /// <param name="sigData">Signature data bytes to send. <para>要发送的签名数据字节。</param>
    ...
    /// </summary>
    /// <param name="sigStream">Stream containing signature data. <para>包含签名数据的流。</param>
    /// <param name="length">Length of signature data in bytes. <para>签名数据字节长度。</param>
    /// <returns>A FastbootResponse indicating the result. <para>指示操作结果的 FastbootResponse。</para></returns>
    ...
    /// 读取签名文件并将其发送到设备。</para>
    /// </summary>
    /// <param name="filePath">Path to the signature file. <para>签名文件的路径。</param>
    /// <returns>A FastbootResponse indicating the result. <para>指示操作结果的 FastbootResponse。</para></returns>
