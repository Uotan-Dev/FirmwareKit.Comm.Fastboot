...
namespace FirmwareKit.Comm.Fastboot
{
    ...
    public class UdpTransport : IFastbootTransport
    {
        ...
        public UdpTransport(string host, int port = 5554)
        {
            Host = host;
            Port = port;
        ...
        {
            byte[] response = SendDataInternal(PacketId.DeviceQuery, Array.Empty<byte>(), 0, MaxTransmissionAttempts);
            ...
            // Version (0x0001), Max Packet Size (0x0200 = 2048 or higher)
            byte[] initData = new byte[4];
            BinaryPrimitives.WriteUInt16BigEndian(initData.AsSpan(0, 2), 0x0001);
            BinaryPrimitives.WriteUInt16BigEndian(initData.AsSpan(2, 2), 2048); // Host max packet size

            response = SendDataInternal(PacketId.Initialization, initData, initData.Length, MaxTransmissionAttempts);
